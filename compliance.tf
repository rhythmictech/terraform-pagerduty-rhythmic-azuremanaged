...
resource "pagerduty_jira_cloud_account_mapping_rule" "compliance" {
  ...
  config {
    service = pagerduty_service.compliance.id
    jira {
      create_issue_on_incident_trigger = data.azurerm_key_vault_secret.jira_compliance["create-issue-on-incident-trigger"].value
      ...
      dynamic "custom_fields" {
        for_each = coalesce(jsondecode(nonsensitive(data.azurerm_key_vault_secret.jira_compliance["custom-jira-fields"].value)), [])
      ...
      dynamic "custom_fields" {
        for_each = coalesce(jsondecode(nonsensitive(data.azurerm_key_vault_secret.jira_compliance["custom-fixed-fields"].value)), [])
      ...
      issue_type {
        id   = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-type"].value)[0])
        name = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-type"].value)[1])
      }
      ...
      project {
        id   = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["project"].value)[0])
        key  = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["project"].value)[1])
        name = data.azurerm_key_vault_secret.jira_compliance["project-name"].value
      }

      status_mapping {

        acknowledged {
          id   = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-status-acknowledged"].value)[0])
          name = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-status-acknowledged"].value)[1])
        }
        resolved {
          id   = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-status-resolved"].value)[0])
          name = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-status-resolved"].value)[1])
        }
        triggered {
          id   = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-status-open"].value)[0])
          name = nonsensitive(split(":", data.azurerm_key_vault_secret.jira_compliance["issue-status-open"].value)[1])
        }
      }
